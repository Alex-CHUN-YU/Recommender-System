...
class Statistic():
	...
	# 主要做分類統計
	def main(self):
		self.cursor.execute("SELECT id FROM articles Where id >= 1 and relationship_type !=''")
		articles = self.cursor.fetchall()
		for article in articles:
			article_id = article[0]
			print("id:" + str(article_id))
			try:
				self.statistic(article_id)
			except:
	...
	# 進行 character_object, time, location 以及 emotion, event 的統計
	def statistic(self, id):
